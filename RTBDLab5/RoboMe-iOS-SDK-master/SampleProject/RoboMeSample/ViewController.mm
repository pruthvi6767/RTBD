...
- (void)perform:(NSString *)command {
    ...
    NSArray *input=[command componentsSeparatedByString:@"xyztagxyz"];
    NSLog(@"The Recipe is %@",input[0]);
    NSLog(@"The URL is %@",input[1]);
    NSLog(@"The Rating is %@",input[2]);
    
        _tweetViewer.text = input[0];
        [_tweetViewer setNeedsDisplay];
        _htmlViewer.text=[input objectAtIndex:1];
        [_htmlViewer setNeedsDisplay];
        _ratingViewer.text=[input objectAtIndex:2];
        [_ratingViewer setNeedsDisplay];
        
        
        
   
    
    // _tweetViewer.text=[input objectAtIndex:0];
    //_htmlViewer.text=[input objectAtIndex:1];
    //_ratingViewer.text=[input objectAtIndex:2];
    //    _top30Label.text=input[2];
    


    
    NSString *cmd = [command uppercaseString];
    _textLabel.text=command;
    NSString *textTable = [NSString stringWithFormat: @"%@\n%@", self.textTable.text, cmd];
    
    // print command to output box
    [self.textTable setText: textTable];
    
    // scroll to bottom
    [self.textTable scrollRangeToVisible:NSMakeRange([self.textTable.text length], 0)];
    ...
    else if ([input[2] isEqualToString:@" one"])
    {
        _ratingView.image=[UIImage imageNamed:@"1_star.png"];
    }
    else if ([input[2] isEqualToString:@" two"])
    {
        _ratingView.image=[UIImage imageNamed:@"2_stars.png"];
    }
    else if ([input[2] isEqualToString:@" three"])
    {
        _ratingView.image=[UIImage imageNamed:@"3_stars.png"];
    }
    else if ([input[2] isEqualToString:@" four"])
    {
        _ratingView.image=[UIImage imageNamed:@"4_stars.png"];
    }
    else if ([input[2] isEqualToString:@" five"])
    {
        _ratingView.image=[UIImage imageNamed:@"5_stars.png"];
    }
