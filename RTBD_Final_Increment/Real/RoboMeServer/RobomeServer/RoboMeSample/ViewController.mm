...
- (void)viewDidLoad {
    ...
    [self.outputTextView scrollRangeToVisible:NSMakeRange(0, 1)];
...
- (void)perform:(NSString *)command {
    ...
    if ([cmd isEqualToString:@"LEFT"]) {
        ...
        NSString *city = @"Kansas City";
        NSString *urlrest=@",us&appid=a2f79b1f1eaa40a6837368badad5eb92";
        AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
        city=array[1];
        if (array[2]) {
            city = [NSString stringWithFormat:@"%@%@",array[1],array[2]];
            }
        NSLog(@"%@",city);
        NSLog(@"The City is %@",city);
        
        NSString *url = [NSString stringWithFormat:@"%@%@%@", BASEURL, city, urlrest];
        NSLog(@"%@", url);
        ...
        [manager GET:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
         ...
         //   NSDictionary   *total =responseObject   ;
            ...
            AVSpeechUtterance *utterance = [[AVSpeechUtterance alloc] initWithString:recipetitle];
            utterance.rate = 0.4;
            [self.synthesizer speakUtterance:utterance];
            ...
            [manager GET:recipeurl parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                NSLog(@"The Recipe is %@",recipe);
                [self displayText:FORMAT(@"The Source URL is %@ \n Ingredients Required:- \n",recipesource_url)];
                self.synthesizer = [[AVSpeechSynthesizer alloc] init];

                for (int i=0; i<([recipe count]); i++) {
                    
                    AVSpeechUtterance *utterance = [[AVSpeechUtterance alloc] initWithString:recipe[i]];
                    utterance.rate = 0.5;
                    [self.synthesizer speakUtterance:utterance];
                ...
                for (int i=0; i<(sizeof(recipe)); i++) {
                    NSLog(@"%d",i);
                    NSLog(@"%d",[recipe count]);
                }
                _outputTextView.text = [recipe componentsJoinedByString:@"\n"];
                */
                 }
